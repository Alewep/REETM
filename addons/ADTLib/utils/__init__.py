     ...
     def weight_bias_init(self):
         ...
         if self.init_method=='zero':
            self.biases = tf.Variable(tf.zeros([self.n_classes]))           
               ...
               self.biases = tf.Variable(tf.random.normal([self.n_classes]))           
         if self.configuration =='B':
             if self.init_method=='zero':  
                 self.weights =tf.Variable(tf.random.normal([self.n_hidden[(len(self.n_hidden)-1)]*2, self.n_classes]))
             elif self.init_method=='norm':
                   self.weights = { '1': tf.Variable(tf.random.normal([self.n_hidden[(len(self.n_hidden)-1)], self.n_classes])),'2': tf.Variable(tf.random.normal([self.n_hidden[(len(self.n_hidden)-1)], self.n_classes]))} 
         if self.configuration =='R':
             if self.init_method=='zero':  
                 self.weights = tf.Variable(tf.random.normal([self.n_hidden[(len(self.n_hidden)-1)], self.n_classes]))     
             elif self.init_method=='norm':
                   self.weights = tf.Variable(tf.random.normal([self.n_hidden[(len(self.n_hidden)-1)], self.n_classes]))
      
     def attention_weight_init(self,num):
         if num==0:
             self.attention_weights=[tf.Variable(tf.random.normal([self.n_hidden[(len(self.n_hidden)-1)]*4,self.n_hidden[(len(self.n_hidden)-1)]*2]))]
             self.sm_attention_weights=[tf.Variable(tf.random.normal([self.n_hidden[(len(self.n_hidden)-1)]*2,self.n_hidden[(len(self.n_hidden)-1)]*2]))]
         if num>0:
             self.attention_weights.append(tf.Variable(tf.random.normal([self.n_hidden[(len(self.n_hidden)-1)]*4,self.n_hidden[(len(self.n_hidden)-1)]*2])))
             self.sm_attention_weights.append(tf.Variable(tf.random.normal([self.n_hidden[(len(self.n_hidden)-1)]*2,self.n_hidden[(len(self.n_hidden)-1)]*2])))
     def create(self):
       
       tf.compat.v1.reset_default_graph()
       self.weight_bias_init()
       self.x_ph = tf.compat.v1.placeholder("float32", [1, self.batch.shape[0], self.batch.shape[1]])
       self.y_ph = tf.compat.v1.placeholder("float32", self.batch_targ.shape)
       self.seq=tf.constant(self.truncated,shape=[1])
       self.seq2=tf.constant(self.truncated,shape=[1]) 
       self.dropout_ph = tf.compat.v1.placeholder("float32")
       self.fw_cell=self.cell_create('1')
       self.fw_cell2=self.cell_create('2')
       if self.configuration=='R':
           self.outputs, self.states= tf.compat.v1.nn.dynamic_rnn(self.fw_cell, self.x_ph,
                                             sequence_length=self.seq,dtype=tf.float32)
           if self.attention_number >0:
               self.outputs_zero_padded=tf.pad(tensor=self.outputs,paddings=[[0,0],[self.attention_number,0],[0,0]])
           ...
           with tf.compat.v1.variable_scope('1'):
               self.outputs, self.states= tf.compat.v1.nn.bidirectional_dynamic_rnn(self.fw_cell, self.bw_cell, self.x_ph,
                                             sequence_length=self.seq,dtype=tf.float32)
                                              
           self.first_out=tf.concat((self.outputs[0],self.outputs[1]),2)
           with tf.compat.v1.variable_scope('2'):
               self.outputs2, self.states2= tf.compat.v1.nn.bidirectional_dynamic_rnn(self.fw_cell2, self.bw_cell2, self.first_out,
           ...
           self.zero_pad_second_out=tf.pad(tensor=tf.squeeze(self.second_out),paddings=[[self.attention_number,self.attention_number],[0,0]])
#               self.attention_chunks.append(self.zero_pad_second_out[j:j+attention_number*2])
           ...
           self.attention_z=tf.reduce_sum(input_tensor=[self.attention_s[i]*self.zero_pad_second_out[i:self.batch_size+i] for i in range(self.attention_number*2+1)],axis=0)
       ...
       if  self.output_act=='softmax':   
           ...
           self.cost = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=self.presoft, labels=tf.stop_gradient(self.y_ph)))
       elif self.output_act=='sigmoid':
           self.pred=tf.nn.sigmoid(self.presoft)
           self.cost = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=self.presoft, labels=self.y_ph))
       if self.optimizer == 'GD':
             self.optimize = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=self.learning_rate).minimize(self.cost)
       elif self.optimizer == 'Adam':
             self.optimize = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.cost) 
       elif self.optimizer == 'RMS':
             self.optimize = tf.compat.v1.train.RMSPropOptimizer(learning_rate=self.learning_rate).minimize(self.cost) 
       self.correct_pred = tf.equal(tf.argmax(input=self.pred,axis=1), tf.argmax(input=self.y_ph,axis=1))
       self.accuracy = tf.reduce_mean(input_tensor=tf.cast(self.correct_pred, tf.float32))
       self.init = tf.compat.v1.global_variables_initializer()
       self.saver = tf.compat.v1.train.Saver()
       self.saver_var = tf.compat.v1.train.Saver(tf.compat.v1.trainable_variables())
